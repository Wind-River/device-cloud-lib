...
#else /* if defined( IOT_JSON_JSMN ) || defined( IOT_JSON_JSONC ) */
/**
...
#endif /* if defined( IOT_JSON_JSMN ) || defined( IOT_JSON_JSONC )*/
	...
	iot_int64_t *value )
{
	...
	if ( decoder && item )
	{
		...
		{
			v = (long)json_object_get_int64( j );
			result = IOT_STATUS_SUCCESS;
		}
#else /* defined( IOT_JSON_JSMN ) */
		const jsmntok_t *const cur = (const jsmntok_t *)item;
	...
	if ( decoder && item )
	{
			...
			json_object_is_type( j, json_type_int ) != FALSE )
		{
			if ( json_object_is_type( j, json_type_double ) != FALSE )
				...
				v = (double)json_object_get_int64( j );
			result = IOT_STATUS_SUCCESS;
		}
#else /* defined( IOT_JSON_JSMN ) */
		result = app_jsmn_decode_number( decoder, item, &v, NULL );
#endif /* defined( IOT_JSON_JSMN ) */
	}
